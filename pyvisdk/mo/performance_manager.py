...
class PerformanceManager(BaseEntity):
    '''This managed object type provides the service interface for obtaining statistical
        ...
        StatsType, RollupType, and Level:* * * * * * * Storage Capacity: * *
        Storage I/O: * * * * * * *Other performance-counter groups, in addition to
        those listed here, exist on the system. However, only the counter groups
        listed are considered of possible interest to third-party developers.This
        interface provides these query operations:* QueryPerfProviderSummary, for
        obtaining metatdata about performance providers * QueryPerfCounter and
        ...
        statistics for a single entity with its descendent objects?statistics for
        a host and all its virtual machines, for example.Some differences between
        ESX and vCenter Server implementations of this interface include:* For ESX
        systems, this interface provides access to real-time data, and to real-
        time data that has been rolled up into "PastDay" statistics (if applicable
        for the specific counter). * For vCenter Server systems, this interface
        provides access to real-time and historical data. vCenter Server collects
        statistics on a regular basis from all ESX systems that it manages, and
        aggregates the results based on the level settings for the server. *
        Default sampling interval is product- and version-specific: * ESX 3.x (and
        subsequent) systems: 20 second interval * ESX 2.x systems: 60 second
        interval * VirtualCenter Server 2.5 (and subsequent vCenter Server)
        systems initially collect statistics data 10 minutes after system startup,
        and then hourly thereafter.See the Programming Guide for more information
        about using PerformanceManager.
    '''

...
class DistributedVirtualSwitchManager(BaseEntity):
    ...
    def QueryCompatibleHostForExistingDvs(self, container, recursive, dvs):
        ...
        :param container: to a ManagedEntityWhere to look for hosts. Supported types of objects for this
        parameter are Datacenter, ComputeResource and Folder.

        :param recursive: Whether to search for hosts in the subfolders, if applicable. In the case when
        container is a Datacenter, the recursive flag is applied to its
        HostFolder.

        :param dvs: to a DistributedVirtualSwitchSearch the host based on the specification published
        in the compatibleHostComponentProductInfo of a DistributedVirtualSwitch.
        If not set, it is assumed to be the specification that a
        DistributedVirtualSwitch would have if it is created with the default
    ...
    def QueryDvsCheckCompatibility(self, hostContainer, dvsProductSpec, hostFilterSpec):
        ...
        hosts in the hierarchy with container as root of the tree.

        :param dvsProductSpec: The productSpec of a DistributedVirtualSwitch. If not set, it is assumed to be the
        default one used for DistributedVirtualSwitch creation for current
        version.

        :param hostFilterSpec: The hosts against which to check compatibility. This is a filterSpec and users can
        use this to specify all hosts in a container (datacenter, folder, or
        computeResource), an array of hosts, or hosts that might or might not be a
        DVS member.
    ...
    def QueryDvsConfigTarget(self, host, dvs):
        ...
        :param host: to a HostSystemThe host on which the query is to be made. If called directly on
        the host this parameter need not be specified.

        :param dvs: to a DistributedVirtualSwitchThe distributed virtual switch on which the query is
        to be made. If unspecified the config target will encompass all the

...
class DistributedVirtualSwitchManager(BaseEntity):
    ...
    def QueryCompatibleHostForExistingDvs(self, container, recursive, dvs):
        ...
        :param container: Where to look for hosts. Supported types of objects for this parameter are
        Datacenter, ComputeResource and Folder.
        ...
        :param dvs: Search the host based on the specification published in the
        compatibleHostComponentProductInfo of a DistributedVirtualSwitch. If not
        set, it is assumed to be the specification that a DistributedVirtualSwitch
        would have if it is created with the default
    ...
    def QueryDvsConfigTarget(self, host, dvs):
        ...
        :param host: The host on which the query is to be made. If called directly on the host this
        parameter need not be specified.

        :param dvs: The distributed virtual switch on which the query is to be made. If unspecified
        the config target will encompass all the distributed virtual switches

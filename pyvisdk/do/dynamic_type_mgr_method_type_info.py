...
def DynamicTypeMgrMethodTypeInfo(vim, *args, **kwargs):
    obj = vim.client.factory.create('ns0:DynamicTypeMgrMethodTypeInfo')

    # do some validation checking...
    if (len(args) + len(kwargs)) < 6:
        raise IndexError('Expected at least 7 arguments got: %d' % len(args))

    required = [ 'name', 'wsdlName', 'version', 'paramTypeInfo', 'fault', 'privId', 'annotation']

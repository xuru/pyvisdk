...
def DynamicTypeMgrPropertyTypeInfo(vim, *args, **kwargs):
    obj = vim.client.factory.create('ns0:DynamicTypeMgrPropertyTypeInfo')

    # do some validation checking...
    if (len(args) + len(kwargs)) < 5:
        raise IndexError('Expected at least 6 arguments got: %d' % len(args))

    required = [ 'name', 'wsdlName', 'type', 'privId', 'msgIdFormat' 'annotation']

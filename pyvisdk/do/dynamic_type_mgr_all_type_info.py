...
def DynamicTypeMgrAllTypeInfo(vim, *args, **kwargs):
    obj = vim.client.factory.create('ns0:DynamicTypeMgrAllTypeInfo')

    # do some validation checking...
    if (len(args) + len(kwargs)) < 2:
        raise IndexError('Expected at least 3 arguments got: %d' % len(args))

    required = [ "managedTypeInfo", "enumTypeInfo", "dataTypeInfo" ]

...
def FcoeConfigFcoeSpecification(vim, *args, **kwargs):
    ...
    obj = vim.client.factory.create('ns0:FcoeConfigFcoeSpecification')

    # do some validation checking...
    if (len(args) + len(kwargs)) < 1:
        raise IndexError('Expected at least 2 arguments got: %d' % len(args))

    required = [ 'underlyingPnic' ]
    optional = [ 'priorityClass', 'sourceMac', 'vlanRange', 'dynamicProperty', 'dynamicType' ]
